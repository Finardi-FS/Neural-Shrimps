...
class PCA_Class:
    def __init__(self, data_int, data_ext):

        self._dict = {}
        self._dict['cov_mat']               = np.corrcoef(data, rowvar=False)
        evl, evc                            = np.linalg.eig(self._cov_mat)
        self._dict['sorted_idcs']           = np.argsort(eig_val)[::-1]
        ...
        sum_ev                              = np.round(np.sum(self._dict['eval']), 1)
        self._dict['eval_var']              = np.abs(self._dict['eval']/sum_ev)
        self._dict['cumulative_var']        = np.cumsum(self._dict['eval_var'])
        return self._dict

    def get_PCA_data(self, t):
        """t: cumulative variance threshold"""
        cumulative = self._dict['cumulative_var']
        PCA_data = {}

        PCA_data['pca_eval'] = self._dict['eval'] [cumulative < t] # prendo gli autovalori che spieghino un tot percentuale (t) della varianza
        PCA_data['pca_evec'] = self._dict['evec'] [:,cumulative < t] # prendo i corrispettivi autovettori

        PCA_data['pc_data_int'] = self._dict['data_int'] @ PCA_data['pca_eval'] # Prodotto scalare tra matrice di dati (righe features, colonne stati) e matrice di autovettori principali disposti in colonna
        PCA_data['pc_data_ext'] = self._dict['data_ext'] @ PCA_data['pca_evec']
...
# -------------------------------------------------------------------------------------------------------------------- #
	...
	def table_kfolds(self, dic: dict):
		jupyprint(pd.DataFrame(dic.values(), index = dic.keys(), columns = ['(K: {})'.format(str(i)+' of '+str(len(dic['FP']))) for i in range(1,len(dic['FP'])+1)]))
...
# -------------------------------------------------------------------------------------------------------------------- #
	...
	def avg_std_kfolds(self, dic: dict, table = False):
		...
		if table:
			jupyprint(pd.DataFrame({'Avg': avg_results['Avg'].values(),'Std': avg_results['Std'].values()}, index = avg_results['Avg'].keys()))
		return avg_results
...
# -------------------------------------------------------------------------------------------------------------------- #
	...
	def confusion_matrix(self, dic: dict, table = True):
		...
		if table:
			jupyprint("### Confusion Matrix:")
			jupyprint(pd.DataFrame(confusion_Matrix, index = ['Actual P', 'Actual N'], columns = ['Predicted P', 'Predicted N']))
